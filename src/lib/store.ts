...
export function useHabits() {
  const [habits, setHabits, ready] = useStored<Habit[]>(KEYS.habits, defaultHabits);
  const [lastDay, setLastDay, dayReady] = useStored<string>(KEYS.habitsDay, todayKey());

  useEffect(() => {
    if (!ready || !dayReady) return;
    const today = todayKey();
    if (lastDay !== today) {
      setHabits((prev) => prev.map((habit) => ({ ...habit, done: false })));
      setLastDay(today);
    }
  }, [ready, dayReady, lastDay, setHabits, setLastDay]);

  return [habits, setHabits, ready && dayReady] as const;
}
...
export function notifyHabit(habit: Habit) {
  if (typeof window === "undefined" || !("Notification" in window)) return false;
  if (Notification.permission !== "granted") return false;
  new Notification(`Study Sync reminder: ${habit.title}`, {
    body: `It's ${habit.time}. Tap Study Sync to keep your routine on track.`,
    icon: "/favicon.ico",
    tag: `studysync-${habit.id}-${todayKey()}`,
  });
  return true;
}

export function useDailyReminderNotifications(habits: Habit[], enabled: boolean) {
  useEffect(() => {
    ...
    const check = () => {
      ...
      const sentKey = `studysync.sentReminders.${stamp}`;
      let sent: string[] = [];
      try { sent = JSON.parse(localStorage.getItem(sentKey) || "[]") as string[]; } catch { sent = []; }

      habits.forEach((habit) => {
        if (habit.done || habit.time !== current || sent.includes(habit.id)) return;
        if (notifyHabit(habit)) {
          sent.push(habit.id);
